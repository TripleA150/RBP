...
echo 'OpenVPN + WireGuard + AmneziaWG + OpenConnect'
...
echo
until [[ "$OPENCONNECT_ENABLE" =~ (y|n) ]]; do
	read -rp 'Enable OpenConnect (AnyConnect protocol)? [y/n]: ' -e -i y OPENCONNECT_ENABLE
done
echo
...
echo
while read -rp 'Enter valid domain name for this OpenConnect server or press Enter to skip: ' -e OPENCONNECT_HOST
do
	[[ -z "$OPENCONNECT_HOST" ]] && break
	[[ -n $(getent ahostsv4 "$OPENCONNECT_HOST") ]] && break
done
echo
...
systemctl disable --now ocserv
systemctl disable --now ocserv.socket
systemctl disable --now ocserv@antizapret
systemctl disable --now ocserv@vpn
...
git clone https://github.com/TripleA150/RBP.git /tmp/antizapret
...
mkdir -p /tmp/antizapret/setup/etc/ocserv
cp -r /etc/ocserv/certs /tmp/antizapret/setup/etc/ocserv/ || true
cp /etc/ocserv/*.passwd /tmp/antizapret/setup/etc/ocserv/ || true
cp /etc/ocserv/secrets /tmp/antizapret/setup/etc/ocserv/ || true
...
cp -r /root/ocserv/* /tmp/antizapret/setup/etc/ocserv/ || true
...
OPENCONNECT_ENABLE=$OPENCONNECT_ENABLE
...
if [[ "$VPN_DNS" == '3' ]]; then
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 9.9.9.10\ndns = 149.112.112.10' /etc/ocserv/templates/vpn.conf
elif [[ "$VPN_DNS" == '4' ]]; then
	# Yandex
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 77.88.8.8\ndns = 77.88.8.1' /etc/ocserv/templates/vpn.conf
elif [[ "$VPN_DNS" == '5' ]]; then
	# Google
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 8.8.8.8\ndns = 8.8.4.4' /etc/ocserv/templates/vpn.conf
elif [[ "$VPN_DNS" == '6' ]]; then
	# AdGuard
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 94.140.14.14\ndns = 94.140.15.15' /etc/ocserv/templates/vpn.conf
elif [[ "$VPN_DNS" == '7' ]]; then
	# Comss
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 83.220.169.155\ndns = 212.109.195.93\ndns = 195.133.25.16' /etc/ocserv/templates/vpn.conf
elif [[ "$VPN_DNS" == '8' ]]; then
	# XBox
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 111.88.96.50\ndns = 111.88.96.51' /etc/ocserv/templates/vpn.conf
elif [[ "$VPN_DNS" == '9' ]]; then
	# Malw
	...
	sed -i '/^dns = 1\.1\.1\.1$/,+1c dns = 95.216.204.218\ndns = 80.253.249.40' /etc/ocserv/templates/vpn.conf
fi
