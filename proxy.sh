...
until [[ "$OPENCONNECT" =~ (y|n) ]]; do
	read -rp 'Enable OpenConnect proxying? [y/n]: ' -e -i y OPENCONNECT
done
echo
...
if [[ "$OPENCONNECT" == 'y' ]]; then
	while read -rp 'Enter OpenConnect server IPv4 address: ' -e OPENCONNECT_IP
	do
		[[ -n $(getent ahostsv4 "$OPENCONNECT_IP") ]] || continue
		break
	done
	echo
fi
...
# OpenConnect
if [[ "$OPENCONNECT" == 'y' ]]; then
	iptables -w -t nat -A PREROUTING -p tcp --dport 443 -j DNAT --to-destination $OPENCONNECT_IP:53443
	iptables -w -t nat -A PREROUTING -p udp --dport 443 -j DNAT --to-destination $OPENCONNECT_IP:53443
	iptables -w -t nat -A PREROUTING -p tcp --dport 53443 -j DNAT --to-destination $OPENCONNECT_IP:53443
	iptables -w -t nat -A PREROUTING -p udp --dport 53443 -j DNAT --to-destination $OPENCONNECT_IP:53443
	iptables -w -t nat -A PREROUTING -p tcp --dport 53080 -j DNAT --to-destination $OPENCONNECT_IP:53080
	iptables -w -t nat -A PREROUTING -p udp --dport 53080 -j DNAT --to-destination $OPENCONNECT_IP:53080
fi
# SNAT
