...
if [[ -z "$1" || "$1" == 'ip' || "$1" == 'ips' || "$1" == 'noclear' || "$1" == 'noclean' ]]; then
	...
	echo "route = $FAKE_IP.0.0/255.254.0.0" > result/ocserv-routes.conf
	while read -r cidr; do
		...
		echo "route = $NET/$MASK" >> result/ocserv-routes.conf
	...
	# Создаем конфигурацию OpenConnect из шаблонов и списка маршрутов
	if [[ -d /etc/ocserv/templates ]]; then
		if [[ -f /etc/ocserv/templates/antizapret.conf ]]; then
			cat /etc/ocserv/templates/antizapret.conf result/ocserv-routes.conf > result/ocserv-antizapret.conf
